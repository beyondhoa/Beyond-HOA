...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex flex-col h-screen w-full bg-background select-none overflow-hidden">
      
      {/* 1. MOBILE TOP HEADER (With Top Safe Area) */}
      <header className="flex-none h-[calc(3.5rem+env(safe-area-inset-top))] pt-[env(safe-area-inset-top)] bg-indigo-950 px-4 flex items-center justify-between border-b border-indigo-900/60 shadow-md fine:hidden z-30">
      ...
      {/* 2. DESKTOP SIDEBAR */}
      <aside className="hidden fine:flex w-64 flex-shrink-0 bg-indigo-950 flex-col shadow-xl border-r border-indigo-900/40 pt-[env(safe-area-inset-top)] pb-[env(safe-area-inset-bottom)]">
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-y-auto pb-24 bg-background text-foreground">
        <div className="flex-1 flex flex-col w-full max-w-3xl mx-auto">  {/* ← add max-w-3xl mx-auto */}
          {children}
        </div>
      </main>

      {/* 4. MOBILE BOTTOM TAB BAR (With Dynamic Bottom Safe Area Height) */}
      <nav className="fixed bottom-0 left-0 right-0 h-[calc(4rem+env(safe-area-inset-bottom))] bg-indigo-950 border-t border-indigo-900/60 flex items-center justify-around z-50 fine:hidden pb-[env(safe-area-inset-bottom)] shadow-lg">
        {navItems.map((item) => {
          const Icon = item.icon;
          const isActive =
            location === item.href ||
          ...
          return (
            ...
            >
              ...
              <span className="text-sm mt-1 tracking-tight truncate max-w-[64px]">
                {item.label}
              </span>
            </Link>
          );
