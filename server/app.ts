...
function setupCors(app: express.Application) {
  app.use((req, res, next) => {
    ...
    // Vercel provides the deployment URL (preview + production) via VERCEL_URL,
    ...
    const isVercelPreview =
      !!origin && origin.startsWith("https://") && origin.endsWith(".vercel.app");
...
function getDeployedBaseUrl(): string | null {
  // Replit production deployment
  if (process.env.REPLIT_DOMAINS) {
    return `https://${process.env.REPLIT_DOMAINS.split(",")[0]}`;
  }
  // Vercel production deployment (stable domain) takes priority over preview URL
  if (process.env.VERCEL_PROJECT_PRODUCTION_URL) {
...
async function initStripe() {
  ...
  try {
    ...
    const webhookBaseUrl = getDeployedBaseUrl();
    if (webhookBaseUrl) {
